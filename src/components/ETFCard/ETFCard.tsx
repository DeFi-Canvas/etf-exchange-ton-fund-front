...
const EtfCard = ({
    ...
    const price =
        (useAppSelector((state) =>
            state?.appSlice?.wallet_info?.jettons?.find(
                (j) => j.jetton === address
            )
        )?.balance || 0) * 0.0208;
    ...
    return (
        <Link to={`/funds/${jettonSymbol}`} className={'etf-card'}>
            {badgeText && (
                <BigBadge text={badgeText} className={'etf-card-badge'} />
            )}
            <div className={'etf-card-img'}>
            ...
            {personEtfBalance !== undefined && etfPriceChange !== undefined && (
