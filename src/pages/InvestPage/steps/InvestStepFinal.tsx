...
const InvestStepFinal = () => {
  ...
  useEffect(() => {
    if (wallet?.account.address) {
      dispatch(callContract({tonui, wallet:toUserFriendlyAddress(wallet.account.address)}))
        .then(() => {
          setStatus('confirm')
  ...
  const renderStatus = () => {
    switch (status) {
      ...
      case 'confirm':
        return <>
          <SuccessIcon/>
          <p>{selectedCoinToInvest} have been invested successfully!</p>
          <ButtonPrimary text={'View home page'} onClick={() => navigator('/')}/>
        </>
      case 'error':
        return <>
